...
const appendWeatherCard = function (city, weather) {
  ...
  if (fahrenheit.classList.contains("unit__display")) {
    temp.textContent = `${Math.trunc(weather.main.temp)}°F`;
  } else {
...
const unitBtn = document.querySelector(".unit__btn");
const celsius = document.querySelector(".celsius");
const fahrenheit = document.querySelector(".fahrenheit");

unitBtn.addEventListener("click", unitDisplay);

function unitDisplay() {
  const tempDisplay = document.querySelectorAll(".temp");

  if (fahrenheit.classList.contains("unit__display")) {
    tempDisplay.forEach((v) => {
      v.textContent = `${Number(v.textContent.replace("°F", "")) - 272}°C`;
    });
    fahrenheit.classList.remove("unit__display");
    celsius.classList.add("unit__display");
  } else {
    tempDisplay.forEach((v) => {
      v.textContent = `${Number(v.textContent.replace("°C", "")) + 272}°F`;
    });
    fahrenheit.classList.add("unit__display");
    celsius.classList.remove("unit__display");
  }
}
